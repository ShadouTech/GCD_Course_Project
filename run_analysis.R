...
# Initialize train set (take them out of the UCI folder). Provide headings improve readability.
...
#Load features table and extract headings
features = read.table("./features.txt")
headings = features$V2

#Provide these headings to xTrain and XTest sets
colnames(xTrain) = headings
colnames(xTest) = headings

#Change name of y set to something descriptive
yTest <- rename(yTest, c(V1="activity"))
yTrain <- rename(yTrain, c(V1="activity"))

# change data values in yTest according to activity_labels.txt file, in lowercase
...
tidyData = mergedData[,grep(pattern , names(mergedData), value=TRUE)]
...
write.table(result, file="wearabledataShadouTechCP.txt", sep = "\t", append=F, row.names = FALSE)